(ns amtrak.main
  "Kotodama WASM entrypoint for the Amtrak clean-room actor (L4) — Clojure port.
  ...
  facts (`amtrak.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "amtrak")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"amtrak.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "amtrak-compat" :tier tier :entities entities} 200])
...
(defn start! [] :amtrak-compat/ready)
